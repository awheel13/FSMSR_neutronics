...
for r in range(100, 415, 15):
	...
	res_file = '%d/final_very_long/msfr_res.m' % r
	...
	for line in readout:
		...
		elif re.search('FWD_ANA_BETA_ZERO', line):
			# eff - ADJ_NAUCHI_BETA_EFF 
			# not - FWD_ANA_BETA_ZERO
			kline = line
...
for i in range(len(vol)):
	...
	u_fiss_ratio.append(u238[i]/u235[i])
	u_fiss_err.append(u_fiss_ratio[i]*(np.sqrt(u235_err[i]**2 + u238_err[i]**2))) 
...
## colors ##
num = len(radii)
enrich_colors = pl.cm.ocean(np.linspace(0, 1,(num)))
sph_33ucl_color = pl.cm.ocean(0.85)

#### make sure K is level ####

plt.scatter(radii, keff, linewidth=0.6, marker='p', edgecolors='k', color=enrich_colors)
...
plt.scatter(radii, ench, marker='p', linewidth=0.6, edgecolors='k', color=sph_33ucl_color, s=50)
plt.plot(radii_line, curv_fit_dat1, ls='--', c=sph_33ucl_color, lw=2, zorder=0)
plt.xlabel('Radius of the Core (cm)')
#plt.xlabel('volume (cc)')
...
#### Plot fission fraction ####

plt.errorbar(radii, u_fiss_ratio, yerr=u_fiss_err, marker='p', mec='k', mew=0.6, ms=7, linewidth=2, color=sph_33ucl_color)
plt.xlabel('Radius of the Core (cm)')
plt.ylabel('U-238 Fission/U-235 Fissions')
plt.show()
...
# for x, y, e, color in zip(radii, beta, beta_err, enrich_colors):
    # plt.plot(x, y, 's', color=color, mec='k', mew=0.8, ms=8)
    # plt.errorbar(x, y, e, lw=1, capsize=3, color=color, elinewidth=2)



plt.errorbar(radii, beta, yerr=beta_err, marker='p', mec='k', mew=0.6, ms=7, linewidth=2, color=sph_33ucl_color)
plt.xlabel('Radius of the Core (cm)')
...
plt.errorbar(radii, mngt, yerr=mntg_err, marker='p', mec='k', mew=0.6, ms=7, linewidth=2, color=sph_33ucl_color)
plt.ticklabel_format(axis="y", style="sci", scilimits=(0,0))
plt.xlabel('Radius of the Core (cm)')
#plt.xlabel('volume (cc)')
...
filename.write('sph_enrich_33ucl = [') 
