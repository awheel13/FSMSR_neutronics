...
mycore.save_deck()
    '''
    def __init__(self, r:float=300.0, refl:float=500.0, e:float=0.1083, e2:float=0.1083, salt="58%NaCl+42%UCl3", Ag_r:float=-1.0):
        if r<10.0 or refl<r or e>1.0 or e<0.0 or e2>1.0 or e2<0.0:  # Reject bad input
            raise ValueError("Bad parameters: ", r, refl, e, e2)

        MSFRbase.__init__(self)         # in Python the parent class needs explicit initialization
        # core parameters
        self.r:float           = r      # Core radius [cm]
        self.h:float           = 2*r      # Core height [cm]
        self.refl:float        = refl   # Outer reflector thickness [cm]
        self.refl_h:float      = 2*refl # Outer height of the reflector [cm]
        ...
        self.re_s              = Salt(self.salt_formula, e2) # refuel salt
        self.s.set_chlorine_37Cl_fraction(0.99999)      # Enriched chlorine-37
        self.re_s.set_chlorine_37Cl_fraction(0.99999)
...
surf 1   cyl  0.0 0.0 {self.r} -{self.r} {self.r}      % fuel salt radius'''
        if self.silver_at_r <= self.r:
            surfaces += '''
...
'''
    ...
    def get_repr_cards(self) -> str:
        ...
        refuel_tmp   = self.re_s.serpent_mat(self.tempK) # Same as fresh fuel
...
'''
    ...
    def get_depl_add_3years(self) -> str:
        'Add 3 years in daysteps'
        depl_add_3years = '''\
...
14 14 14 14 14 14 14 14 14 14 14 14 14 % 182 days
'''
        return depl_add_3years.format(**locals())
