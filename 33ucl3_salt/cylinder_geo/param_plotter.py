...
fig, ax = plt.subplots()
for j in range(len(radii)):
	plt.plot(burn_years[j, :], keff[j, :], linewidth=1, c=refuel_colors[j])
...
### Mean neutonr generatino time ###
fig, ax = plt.subplots()
for j in range(len(radii)):
	plt.errorbar(burn[j, :], Lam[j, :], yerr=Lam_err[0, :], marker='o', mec='k', mew=0.6, ms=2, linewidth=2, color=refuel_colors[j], label=label_str[j])
plt.xlabel('Burnup (MWd/kgU)')
plt.ylabel('Total Neutron Lifetime')
plt.ticklabel_format(axis="y", style="sci", scilimits=(0,0))
cmap = mcolors.LinearSegmentedColormap.from_list('my_cmap', refuel_colors)
...
fig, ax = plt.subplots()
for j in range(len(radii)):
	plt.errorbar(burn[j, :], beta[j, :], yerr=beta_err[0, :], marker='o', mec='k', mew=0.6, ms=2, linewidth=2, color=refuel_colors[j], label=label_str[j])
plt.xlabel('Burnup (MWd/kgU)')
plt.ylabel('Effective Delayed Neutron Fraction')
cmap = mcolors.LinearSegmentedColormap.from_list('my_cmap', refuel_colors)
norm = mcolors.Normalize(min(radii), max(radii))
