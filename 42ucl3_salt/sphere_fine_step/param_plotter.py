...
u235_err  = np.zeros((rows, step_count))
u238   = np.zeros((rows, step_count))
u238_err  = np.zeros((rows, step_count))
pu239  = np.zeros((rows, step_count)) # PU239_FISS
pu239_err = np.zeros((rows, step_count))
pu240  = np.zeros((rows, step_count)) # PU240_FISS
pu240_err  = np.zeros((rows, step_count)) # PU240_FISS
pu241  = np.zeros((rows, step_count)) # PU241_FISS
pu241_err  = np.zeros((rows, step_count)) # PU241_FISS
...
for r in radii:
	...
	data_file = '%d/rep/first_long/msfr_res.m' % r
	...
	for line in readout:
		if re.search('ABS_KEFF', line):
			...
			kline          = line
			ksplit         = kline.split()
			u235[i, step]  = float(ksplit[8])
			u235_err[i, step] = float(ksplit[8])*float(ksplit[9])
		elif re.search('U238_FISS', line):
			kline          = line
			ksplit         = kline.split()
			u238[i, step]  = float(ksplit[8])
			u238_err[i, step] = float(ksplit[8])*float(ksplit[9])
		elif re.search('PU239_FISS', line):
			kline          = line
			ksplit         = kline.split()
			pu239[i, step] = float(ksplit[8])
			pu239_err[i, step] = float(ksplit[8])*float(ksplit[9])
		elif re.search('PU240_FISS', line):
			kline          = line
			ksplit         = kline.split()
			pu240[i, step] = float(ksplit[8])
			pu240_err[i, step] = float(ksplit[8])*float(ksplit[9])
		elif re.search('PU241_FISS', line):
			kline          = line
			ksplit         = kline.split()
			pu241[i, step] = float(ksplit[8])
			pu241_err[i, step] = float(ksplit[8])*float(ksplit[9])
...
life[ life==0 ] = np.nan
life_err[ life_err==0 ] = np.nan
pu239[ pu239==0 ] = np.nan
pu239_err[ pu239_err==0 ] = np.nan
...
refuel_colors = pl.cm.plasma(np.linspace(0, 0.95,(rows)))
...
fig.savefig('42_sph_iso_burnup.png', dpi=100)
plt.show()
...
#plt.legend()
fig.set_size_inches(14, 10)
fig.savefig('42_sph_iso_burndays.png', dpi=100)
plt.show()


### U 235 fission fraction over burnu ###p
fig, ax = plt.subplots()
for j in range(len(radii)):
	plt.errorbar(burn[j, :], u235[j, :], yerr=u235_err[0, :], marker=',', mec='k', mew=0.6, ms=2, linewidth=1, color=refuel_colors[j], label=label_str[j])

plt.xlabel('Burnup (MWd/kgU)')
plt.ylabel('U-235 Fraction of fission')
cmap = mcolors.LinearSegmentedColormap.from_list('my_cmap', refuel_colors)
norm = mcolors.Normalize(min(radii), max(radii))
sm = matplotlib.cm.ScalarMappable(cmap=cmap, norm=norm)
sm.set_array([])
...
#plt.legend()
fig.set_size_inches(14, 10)
fig.savefig('42_sph_iso_235_fiss.png', dpi=100)
plt.show()


### U 238 fission fraction over burnu ###p
fig, ax = plt.subplots()
for j in range(len(radii)):
	plt.errorbar(burn[j, :], u238[j, :], yerr=u238_err[0, :], marker=',', mec='k', mew=0.6, ms=2, linewidth=1, color=refuel_colors[j], label=label_str[j])

plt.xlabel('Burnup (MWd/kgU)')
plt.ylabel('U-238 Fraction of fission')
cmap = mcolors.LinearSegmentedColormap.from_list('my_cmap', refuel_colors)
norm = mcolors.Normalize(min(radii), max(radii))
sm = matplotlib.cm.ScalarMappable(cmap=cmap, norm=norm)
sm.set_array([])
...
#plt.legend()
fig.set_size_inches(14, 10)
fig.savefig('42_sph_iso_238_fiss.png', dpi=100)
plt.show()


### Pu 239 fission fraction over burnu ###p
fig, ax = plt.subplots()
for j in range(len(radii)):
	plt.errorbar(burn[j, :], pu239[j, :], yerr=pu239_err[0, :], marker=',', mec='k', mew=0.6, ms=2, linewidth=1, color=refuel_colors[j], label=label_str[j])

plt.xlabel('Burnup (MWd/kgU)')
plt.ylabel('Pu-239 Fraction of fission')
cmap = mcolors.LinearSegmentedColormap.from_list('my_cmap', refuel_colors)
norm = mcolors.Normalize(min(radii), max(radii))
sm = matplotlib.cm.ScalarMappable(cmap=cmap, norm=norm)
sm.set_array([])
...
#plt.legend()
fig.set_size_inches(14, 10)
fig.savefig('42_sph_iso_239_fiss.png', dpi=100)
plt.show()

### Pu 240 fission fraction over burnu ###p
fig, ax = plt.subplots()
for j in range(len(radii)):
	plt.errorbar(burn[j, :], pu240[j, :], yerr=pu240_err[0, :], marker=',', mec='k', mew=0.6, ms=2, linewidth=1, color=refuel_colors[j], label=label_str[j])

plt.xlabel('Burnup (MWd/kgU)')
plt.ylabel('Pu-240 Fraction of fission')
cmap = mcolors.LinearSegmentedColormap.from_list('my_cmap', refuel_colors)
norm = mcolors.Normalize(min(radii), max(radii))
sm = matplotlib.cm.ScalarMappable(cmap=cmap, norm=norm)
sm.set_array([])
...
#plt.legend()
fig.set_size_inches(14, 10)
fig.savefig('42_sph_iso_240_fiss.png', dpi=100)
plt.show()



### Pu 241 fission fraction over burnu ###p
fig, ax = plt.subplots()
for j in range(len(radii)):
	plt.errorbar(burn[j, :], pu241[j, :], yerr=pu241_err[0, :], marker=',', mec='k', mew=0.6, ms=2, linewidth=1, color=refuel_colors[j], label=label_str[j])

plt.xlabel('Burnup (MWd/kgU)')
plt.ylabel('Pu-241 Fraction of fission')
cmap = mcolors.LinearSegmentedColormap.from_list('my_cmap', refuel_colors)
norm = mcolors.Normalize(min(radii), max(radii))
sm = matplotlib.cm.ScalarMappable(cmap=cmap, norm=norm)
sm.set_array([])
...
#plt.legend()
fig.set_size_inches(14, 10)
fig.savefig('42_sph_iso_241_fiss.png', dpi=100)
plt.show()
...
#plt.legend()
fig.set_size_inches(14, 10)
fig.savefig('42_sph_iso_lam.png', dpi=100)
plt.show()
...
#plt.legend()
fig.set_size_inches(14, 10)
fig.savefig('42_sph_iso_beta.png', dpi=100)
plt.show()
...
#plt.legend()
fig.set_size_inches(14, 10)
fig.savefig('42_sph_iso_nubar.png', dpi=100)
plt.show()
